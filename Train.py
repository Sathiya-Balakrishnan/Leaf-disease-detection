...
chdir(r'C:/Users/admin/Downloads/plantdisease_dataset')
...
train_g = train_idg.flow_from_directory(directory=r'training',
                                        target_size=(100, 100),
                                        class_mode='binary',
                                        batch_size=125,
                                        shuffle=True)
...
valid_g = valid_idg.flow_from_directory(directory=r'validation',
                                        target_size=(100, 100),
                                        class_mode='binary',
                                        batch_size=125,
                                        shuffle=True)
...
# Settting Callbacks
check_p = callbacks.ModelCheckpoint(filepath='plant_cnn_{val_accuracy:.2f}.h5',
                                    monitor='val_accuracy', verbose=1,
                                    save_best_only=True, save_weights_only=False)
reduce_lr = callbacks.ReduceLROnPlateau(monitor='val_accuracy', factor=0.95, patience=3,
                                        verbose=1, cooldown=2)
callb_l = [check_p, reduce_lr]

# Training Options
fit = my_model.fit_generator(generator=train_g, steps_per_epoch=22, epochs=10, verbose=1,
                             callbacks=callb_l, validation_data=valid_g, validation_steps=4)

# Saving Model
my_model.save(filepath=r'planttom_cnn.h5', overwrite=True)
